...
class PatrolGraph():
        ...
        with open(filepath, "r") as file:
            ...
            for _ in range(self.graphDimension):
                ...
                self.graph.add_node(i,
                    pos = (int(float(file.readline())) * self.resolution,
                        int(float(file.readline())) * self.resolution ),
                    visitTime = 0.0,
                    id = i,
                    nodeType = 0
                )
                ...
                for _ in range(numEdges):
                    ...
                    cost = float(file.readline()) # we no longer use this cost value, as it does not correspond to the actual euclidean distance.
    ...
    def getNodePosition(self, node):
        ...
        #     f.write(f"{x} {y}\n")
        #     f.write(f"{x + self.offsetX} {self.widthPixels * self.resolution + self.offsetY - y}\n\n")


        return (x+self.offsetX , self.widthPixels*self.resolution+self.offsetY - y)
